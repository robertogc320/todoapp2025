        ...
        <div class="w-full">
            ...
            <div>
                <div>
                    ...
                    <x-input type="text" name="name" id="name" wire:model="titulo"
                ...
                <div>
                    ...
                    <textarea name="descripcion" id="descripcion" wire:model="descripcion" value="{{ old('descripcion') }}"
            ...
            @if ($editando)
                <div class="grid grid-cols-2 gap-4">
                    <div wire:click="actualizarTarea()"
                        class="w-full mt-4 px-4 bg-indigo-500 text-white block py-2 rounded-md hover:bg-indigo-600 text-center">
                        Guardar</div>
                    <div wire:click="borrarTarea()" wire:loading.attr="disabled"
                        class="w-full mt-4 px-4 bg-red-500 text-white block py-2 rounded-md hover:bg-red-600 text-center">
                        Borrar</div>
                </div>
            @else
                <div wire:click="guardarTarea()"
                    class="w-full mt-4 bg-sky-400 text-white dark:text-black block py-2 px-2 rounded-md hover:bg-sky-500 text-center cursor-pointer">
                    Registrar</div>
            @endif
        </div>
    </div>
     ...
     bg-white dark:bg-black
        ...
        <form wire:submit="agregarAdjuntos" id="uploadForm">
            ...
            <div x-data="{ uploading: false, progress: 0 }" x-on:livewire-upload-start="uploading = true"
                ...
                <input id="imputTareasFiles" type="file" wire:model="files" name="files"
                    id="upload{{ $iteration }}" multiple class="hidden" accept=".jpg,.pdf,.jpeg">
        ...
        <div id="adjuntos_secction" class="mt-2">
            <div class="grid grid-cols-1">
                <div class="flex flex-row gap-2 ">
                    <div>
                        <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100">Adjuntar Imagenes y/o Pdf´s
                        </h3>
                    </div>
                    <div onclick="subirArchivos(this)" class="w-8 mr-2 cursor-pointer"><img
                            src="{{ asset('icons8-adjuntar-64.png') }}"</img></div>
                </div>
                                        ...
                                        <tr class="bg-white dark:bg-black">
                                            <td scope="row"
                                                ...
                                                <div class="flex items-center space-x-4 mt-2">
                                                    <div class="relative">
                                                        {{--  ES PDF --}}
                                                        @if (str_ends_with($adjuntoFile->getClientOriginalName(), '.pdf'))
                                                            <img src="{{ asset('icons8-pdf-64.png') }}"
                                                                alt="Archivo pdf" class="w-20 h-20 rounded-lg p-1">
                                                            {{--  ES IMAGEN --}}
                                                        @else
                                                            <img src="{{ $adjuntoFile->temporaryUrl() }}"
                                                                alt="{{ $adjuntoFile->getClientOriginalName() }}"
                                                                class="w-20 h-20 rounded-lg p-1">
                                                        @endif
                                                    </div>
                                                    <div class="flex-1">
                                                        <h3 class="text-sm dark:text-white/70 font-medium">
                                                            {{ $adjuntoFile->getClientOriginalName() }}</h3>
                                                    </div>
                                                </div>
                                            </td>
                                            <td class="py-1 text-right">
                                                <div wire:click="quitarAdjuntoNuevo('{{ $loop->index }}')"
                                                    class="w-8">
                                    ...
                                    <tr class="bg-white dark:bg-black">
                                        <td scope="row"
                                            ...
                                            <div class="flex items-center space-x-4 mt-2">
                                                <div class="relative">
                                                    {{--  ES PDF --}}
                                                    @if (str_ends_with($adjuntoOld->archivo, '.pdf'))
                                                        <img src="{{ asset('icons8-pdf-64.png') }}" alt="Archivo pdf"
                                                            class="w-20 h-20 rounded-lg p-1">
                                                        {{--  ES IMAGEN --}}
                                                    @else
                                                        <img src="{{ route('images', ['filename' => $adjuntoOld->archivo]) }}"
                                                            alt="{{ $adjuntoOld->nombre }}"
                                                            class="w-20 h-20 rounded-lg p-1">
                                                    @endif
                                                </div>
                                                <div class="flex-1">
                                                    <h3 class="text-sm dark:text-white/70 font-medium">
                                                        {{ $adjuntoOld->nombre }}</h3>
                                                </div>
                                            </div>
                                        </td>
                                        <td class="py-1 text-right">
                                            <div wire:click="quitarAdjuntoActual('{{ $adjuntoOld->nombre }}')"
                                                class="w-8">
                                                <img src="{{ asset('icons8-cancel-64.png') }}" />
                                            </div>
                                        </td>
                                    ...
                                    <tr class="bg-white dark:bg-black">
                                        <td scope="row"
                                            ...
                                            <div class="flex items-center space-x-4 mt-2">
                                                <div class="relative">
                                                    {{--  ES PDF --}}
                                                    @if (str_ends_with($adjuntoRemoved->archivo, '.pdf'))
                                                        <img src="{{ asset('icons8-pdf-64.png') }}" alt="Archivo pdf"
                                                            class="w-20 h-20 rounded-lg p-1">
                                                        {{--  ES IMAGEN --}}
                                                    @else
                                                        <img src="{{ route('images', ['filename' => $adjuntoRemoved->archivo]) }}"
                                                            alt="{{ $adjuntoRemoved->nombre }}"
                                                            class="w-20 h-20 rounded-lg p-1">
                                                    @endif
                                                </div>
                                                <div class="flex-1">
                                                    <h3
                                                        class="py-1 px-6 text-sm font-normal text-red-600 line-through whitespace-nowrap dark:text-white cursor-pointer">
                                                        {{ $adjuntoRemoved->nombre }}</h3>
                                                </div>
                                            </div>
                                        </td>
                                    </tr>
